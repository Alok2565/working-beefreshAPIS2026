...
function EditWeightUnit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await getWeightUnitById(Number(id));

        const banner = response.data.data;

        setUnitName(banner.unit_name || "");
        setShortName(banner.short_name || "");
        ...
        navigate("/admin/manage/weight-units");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const generatedSlug = unit_name
      .toLowerCase()
      .trim()
      .replace(/^-+|-+$/g, "");

    setShortName(generatedSlug);
  }, [unit_name]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateWeightUnit(Number(id), {
        unit_name,
        short_name,
        status,
      });
      Swal.fire({
        icon: "success",
        title: "Success",
        text: "Weight unit updated successfully",
        timer: 1500,
        showConfirmButton: false,
      });

      navigate(
        "/admin/manage/products/weight-units?success=Weight unit updated successfully",
      );
    } catch (error: any) {
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error?.response?.data?.message || "Error updating weight unit",
      });
    } finally {
      setLoading(false);
    }
            ...
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
              <div>
                ...
                <p className="text-muted mb-0">
                  Update product weight unit details
                </p>
              </div>
