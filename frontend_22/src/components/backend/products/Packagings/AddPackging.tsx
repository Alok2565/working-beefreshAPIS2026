...
function AddPackging() {
  usePageTitle("Add Packaging");
  ...
  useEffect(() => {
    ...
    const generated = packaging_name
      .toLowerCase()
      .trim()
      .replace(/[\s\W-]+/g, "-")
      .replace(/^-+|-+$/g, "");
    setPackagingSlug(generated);
  }, [packaging_name]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Success",
        text: "Packaging added successfully",
        timer: 1500,
        showConfirmButton: false,
      });

      navigate(
        "/admin/manage/products/attribute/packaging-types?success=Packaging added successfully",
      );
    } catch (error: any) {
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error?.response?.data?.message || "Error creating packaging",
      });
    } finally {
      setLoading(false);
    }
