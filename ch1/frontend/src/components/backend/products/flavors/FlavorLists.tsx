...
function FlavorLists() {
  ...
  const handleStatusToggle = async (id: number, currentStatus: boolean) => {
    try {
      const result = await Swal.fire({
        title: currentStatus ? "Deactivate Flavor?" : "Activate Flavor?",
        text: currentStatus
      ...
      await updateFlavorStatus(id);

      Swal.fire({
        icon: "success",
        title: "Updated",
        text: "Flavor status updated successfully",
        timer: 1500,
  ...
  const handleEdit = (id: number) => {
    navigate(`/admin/manage/products/attribute/flavor/edit/${id}`);
  };
  ...
  const handleSoftDelete = async (id: number) => {
    ...
    try {
      await softDeleteFlavor(id);

      Swal.fire({
        icon: "success",
        title: "Deleted",
        text: "Flavor soft deleted successfully",
        timer: 1500,
        showConfirmButton: false,
      });
  ...
  const handleRestore = async (id: number) => {
    ...
    try {
      await restoreFlavor(id);

      Swal.fire({
        icon: "success",
        title: "Restored",
        text: "Flavor restored successfully",
        timer: 1500,
        showConfirmButton: false,
      });
  ...
  const handlePermanentDelete = async (id: number) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Deleted",
        text: "Flavor permanently deleted",
        timer: 1500,
        showConfirmButton: false,
      });
  ...
  const filteredData = flavors.filter((filtItem: Flavor) =>
    filtItem.flavor_name?.toLowerCase().includes(searchTerm.toLowerCase()),
  );
        ...
        <Row className="mb-3">
          <Col xl={12}>
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
              ...
              <Link to="/admin/manage/products/attribute/flavor/add-new">
                <Button
                  style={{
            ...
            <Table bordered hover responsive className="align-middle">
              ...
              <tbody>
                {loading ? (
                  ...
                  currentData.map((flavor: Flavor, index: number) => (
                    <tr key={flavor.id}>
                      <td>{indexOfFirstItem + index + 1}</td>

                      <td className="fw-semibold">{flavor.flavor_name}</td>

                      <td>
                        <Badge bg="dark">{flavor.flavor_slug}</Badge>
                      </td>

                      <td>
                        <Button
                          size="sm"
                          variant={flavor.status ? "success" : "warning"}
                          onClick={() =>
                            handleStatusToggle(flavor.id, flavor.status)
                          }
                          style={{
                            border: "none",
                            display: "flex",
                        ...
                        <div className="d-flex gap-2 flex-wrap">
                          ...
                          {!flavor.is_deleted ? (
                            <Button
                              size="sm"
                              variant="warning"
                              onClick={() => handleSoftDelete(flavor.id)}
                            >
                            ...
                            <Button
                              size="sm"
                              variant="success"
                              onClick={() => handleRestore(flavor.id)}
                            >
                              <RiRefreshLine />
                            </Button>
                          )}
                          ...
                          <Button
                            size="sm"
                            variant="danger"
                            onClick={() => handlePermanentDelete(flavor.id)}
                          >
                            <RiDeleteBin6Fill />
                          </Button>
                        </div>
