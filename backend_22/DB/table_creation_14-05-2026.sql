...
DROP TABLE IF EXISTS product_attribute_mapping CASCADE;
...
CREATE TABLE IF NOT EXISTS products (
    ...
    deleted_by INTEGER REFERENCES users(id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP -- Removed trailing comma here
);
...
CREATE TABLE IF NOT EXISTS product_variants (
    ...
    weight_unit_id INTEGER REFERENCES weight_units(id) ON DELETE SET NULL,
    variant_name VARCHAR(255),
    sku VARCHAR(100) UNIQUE NOT NULL,
    barcode VARCHAR(100),
    weight NUMERIC(10,2),
    ...
    is_deleted BOOLEAN DEFAULT FALSE,
    deleted_at TIMESTAMP NULL,
    created_by INTEGER REFERENCES users(id), -- Completed the cut-off lines
    updated_by INTEGER REFERENCES users(id),
    deleted_by INTEGER REFERENCES users(id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE IF NOT EXISTS product_attribute_mapping (
    id SERIAL PRIMARY KEY,
    -- Consider changing this to variant_id INTEGER NOT NULL REFERENCES product_variants(id)
    product_id INTEGER NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- Kept consistent with other tables
    UNIQUE(product_id, attribute_id, attribute_value_id)
);

-- Index to optimize complex product filtering/searching
CREATE INDEX IF NOT EXISTS idx_prod_attr_mapping 
ON product_attribute_mapping(product_id, attribute_id);
...
CREATE TABLE IF NOT EXISTS product_meta (
    ...
    canonical_url TEXT,
    schema_json JSONB, -- Excellent choice for performance and flexibility
    og_image TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
